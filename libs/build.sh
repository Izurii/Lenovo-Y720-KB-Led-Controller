...
HOTKEY_ADDON_FILE=./hotkey/build/Release/hotkeyAddon.node
LED_ADDON_FILE=./led/build/Release/ledAddon.node

HOTKEY_ADDON_MD5=./hotkey/hotkeyAddon.md5
LED_ADDON_MD5=./led/ledAddon.md5

buildLedAddon() {
	echo "Building LED addon"
	if ! yarn run build-led-addon; then
...
buildHotkeyAddon() {
	...
	if ! yarn run build-hotkey-addon; then
